...
export function loadProductsFetch() {
  //make an HTTP request like using xhr
  //by default it make a GET request
  const promise = fetch("https://supersimplebackend.dev/products")
    .then((response) => {
      //go to this next step when it got a response from the fetch
      return response.json(); //json returns a Promise
      //by returing a Promise
      //it will wait until the Promise to finish before going to the next step
    })
    .then((productsData) => {
    ...
    .catch((error) => {
      ...
      console.log("Unexpected error. Please try again later.1");
    });
  return promise;
}
...
export function loadProducts(fun) {
  ...
  //set up a seperate callback just for errors
...
export const products = [
  ...
  },
  {
    ...
    name: "Mens Black backpack with shoulder pads",
    rating: {
      stars: 5,
      count: 240,
    },
    priceCents: 1999,
    keywords: ["backpack", "rucksack", "bag", "mens"],
  },
  {
    id: "id2",
    image: "images/products/umbrella.jpg",
    name: "Darkgreen Lightweight Umbrella",
    rating: {
      stars: 4.0,
      count: 20,
    },
